...
class DNS1D {
     ...
     * @protected
     */
    protected function getBarcodeHTML($code, $type, $w = 2, $h = 80, $color = 'black', $showCode =false) {
        if (!$this->store_path) {
            $this->setStorPath(app('config')->get("barcode.store_path"));
        }
        $this->setBarcode($code, $type);
	    ...
	    if($showCode)
            ...
            if ($v['t']) {
                $y = round(($v['p'] * $h / $this->barcode_array['maxh']), 3);
                // draw a vertical bar
                $html .= '<div style="background-color:' . $color . ';width:' . $bw . 'px;height:80px;position:absolute;left:' . $x . 'px;top:' . $y . 'px;">&nbsp;</div>' . "\n";
            }
            $x += $bw;
        }
	if($showCode)
